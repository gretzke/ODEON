...
contract('Token', accounts => {
  ...
  it("test initialization", async () => {
    ...
    assert.equal(balance.toNumber(), web3.toWei(44534000, "ether"), "wrong owner balance");
  ...
  it("should fail to transfer 20 tokens from the acc0 to acc1", async () => {
    try {
      let result = await instance.transferFrom(accounts[0], accounts[1], 20 * decimals, {
        from: accounts[1]
      });
  ...
  it("should fail to transfer 10 tokens from the acc0 to acc1 because of wrong sender", async () => {
    try {
      let result = await instance.transferFrom(accounts[0], accounts[1], 10 * decimals, {
        from: accounts[2]
      });
      assert.fail("should have failed");
    } catch (error) {
  ...
  it("should transfer 10 tokens from acc1 to acc2", async () => {
    let result = await instance.transfer(accounts[2], 10 * decimals, {
      from: accounts[1]
    });
  ...
  it("should fail to transfer from acc1 because of insufficient funds", async () => {
    try {
      let result = await instance.transfer(accounts[2], 60 * decimals, {
        from: accounts[1]
      });
      assert.fail("should have failed");
    } catch (error) {
